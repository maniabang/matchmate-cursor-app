...
export default function SignupStep4() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedTags.length > 0 && selectedIdeals.length > 0) {
  ...
  return (
    <div style={{
      ...
      >
        ...
        <button
          type="submit"
          disabled={selectedTags.length === 0 || selectedIdeals.length === 0}
          style={{
            width: '100%',
            padding: '12px 0',
            borderRadius: 16,
            background: selectedTags.length > 0 && selectedIdeals.length > 0 ? '#EBA8A6' : '#E6E6E6',
            color: '#fff',
            fontWeight: 700,
            fontSize: '1.05rem',
            border: 'none',
            boxShadow: '0 2px 8px rgba(22, 12, 12, 0.10)',
            cursor: selectedTags.length > 0 && selectedIdeals.length > 0 ? 'pointer' : 'not-allowed',
            letterSpacing: '-1px',
            marginTop: 8,
