...
export default function MessageDetail({ id, name, profile, messages, myId, onBack }: {
  id: string;
  name: string;
  profile?: Profile;
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] rounded-2xl px-4 py-2 text-sm shadow-sm
                ${isMe ? 'bg-[#EBA8A6] text-white rounded-br-md' : 'bg-white text-gray-900 rounded-bl-md border border-gray-100'}`}
              >
                {msg.content}
