    ...
    <div className="bg-[#F8F8F8] min-h-screen flex flex-col">
        ...
        <div className="flex gap-2 bg-[#F2EAEA] rounded-full p-1 shadow-sm">
          <button
            className={`px-6 py-2 rounded-full text-base font-semibold transition-colors ${
              isReceived ? 'bg-white text-[#EBA8A6] shadow' : 'text-gray-400'
            }`}
      ...
      <div className="flex-1 flex flex-col gap-5 px-4 py-6">
        {list.map((user) => (
          ...
          >
            <div className="absolute right-5 top-1/2 -translate-y-1/2 flex gap-2" style={{ zIndex: 2 }}>
              {isReceived && (
                <>
                  <button className="w-10 h-10 flex items-center justify-center rounded-full bg-[#EBA8A6]/10 text-[#EBA8A6] text-xl hover:bg-[#EBA8A6]/20 transition">
                    ❤️
                  </button>
                  <button className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 text-gray-400 text-xl hover:bg-gray-200 transition">
                    ✖️
                  </button>
                </>
              )}
            </div>
            <div className="flex items-center gap-4 cursor-pointer" onClick={() => router.push(`/profile/${user.id}`)}>
              <div className="w-16 h-16 rounded-full overflow-hidden border border-gray-200 flex-shrink-0">
                <Image
                  src={user.profile && user.profile.trim() !== '' ? user.profile : '/images/profile-default-female.svg'}
                  alt="프로필"
                  width={64}
                  height={64}
              ...
              <div className="flex flex-col min-w-0">
                ...
                <div className="text-sm text-[#EBA8A6] truncate">
                  {user.region}, {user.job}
                </div>
              </div>
            </div>
          </div>
